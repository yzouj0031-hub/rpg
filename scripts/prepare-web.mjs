...
const peerCdnUrl = 'https://unpkg.com/peerjs@1.5.5/dist/peerjs.min.js';
const tracks = [
  {
...
for (const file of ['index.html', 'multiplayer.js', 'sw.js', 'manifest.webmanifest']) {
...
await copyFile(
  path.join(rootDir, 'node_modules', 'peerjs', 'dist', 'peerjs.min.js'),
  path.join(webDir, 'vendor', 'peerjs.min.js')
);
...
serviceWorker = serviceWorker
  .replaceAll('di13jie-v14', 'di13jie-apk-v9')
  .replaceAll(threeCdnUrl, 'vendor/three.min.js')
  .replaceAll(peerCdnUrl, 'vendor/peerjs.min.js');
for (const { url, localPath } of tracks) {
